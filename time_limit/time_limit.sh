...
process_user() {
    ...
    read -r stored_date stored_seconds <"$counter_file"
    current_date=$(date +%F)
    current_time=$(date +%T)
    current_time_h=$(date +%H)

    # Reset if date changed and current hour is > 8
    if [ "$stored_date" != "$current_date" ] && [ "$current_time_h" -gt 8 ]; then
        stored_seconds=0
        echo "$current_date $stored_seconds" >"$counter_file"
    fi

    stored_seconds=$((stored_seconds + CHECK_PERIOD))
    echo "$current_date $stored_seconds" >"$counter_file"

    local log_message="$current_date $current_time User $username active. Time: $stored_seconds/$max_time sec"
    echo "$log_message" >>"$user_log"
    echo "$log_message" >>"$LOG_FILE"

    # Send notification to user
    send_notification "$username" "Achtung Zeitlimit" "Zeit verbraucht: $stored_seconds/$max_time Sek"

    # Kill if over max
    if [ "$stored_seconds" -ge "$max_time" ]; then
        echo "$current_date $current_time TERMINATING user $username (exceeded $max_time seconds)" >>"$LOG_FILE"
